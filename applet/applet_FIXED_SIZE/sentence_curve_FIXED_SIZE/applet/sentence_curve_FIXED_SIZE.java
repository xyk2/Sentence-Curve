...
public class sentence_curve_FIXED_SIZE extends PApplet {
...
public void draw() {
  if (menu == 0) {
    ...
    bezierCountMax = PApplet.parseInt(map(sliderX, 900-120, 900-30, 10, 50)); // set maximum number of beziers to a value mapped from x-value of slider
...
public void keyPressed() { // called whenever a key is pressed
  if (keyCode == BACKSPACE && menu == 0) {
    ...
    for (int x = 0; x<textLength; x++) { // generates character and integer array 
      ...
      intArrayMappedX[x] = PApplet.parseInt(map(intArray[x], 96, 123, 0, 900)); // map integer array into coordinate values that uses full canvas size
      intArrayMappedY[x] = PApplet.parseInt(map(intArray[x], 96, 123, 0, 500));
    }
...
public void generateBezier() {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "sentence_curve_FIXED_SIZE" });
  }
}
