...
void draw() {
  if (menu == 0) {
    ...
    bezierCountMax = int(map(sliderX, screen.width-120, screen.width-30, 10, 50)); // set maximum number of beziers to a value mapped from x-value of slider
...
void keyPressed() { // called whenever a key is pressed
  if (keyCode == BACKSPACE && menu == 0) {
    ...
    for (int x = 0; x<textLength; x++) { // generates character and integer array 
      ...
      intArrayMappedX[x] = int(map(intArray[x], 96, 123, 0, screen.width)); // map integer array into coordinate values that uses full canvas size
      intArrayMappedY[x] = int(map(intArray[x], 96, 123, 0, screen.height));
    }
